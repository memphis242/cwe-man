...
- Seed corpora should include:
  - minimal and maximal valid CWE API JSON payloads
  - malformed/truncated JSON payloads
  - oversized strings and deeply nested arrays/objects
  - invalid UTF-8 and mixed-encoding strings
  - invalid or missing required keys (`Views`, `Categories`, `Weaknesses`, IDs)
  - duplicate or conflicting IDs and relationship edges
  - valid/invalid config key/value combinations
  - edge-case command inputs (`:cwe`, `:print-cwes`, `:clear-runtime`, malformed variants)
  - path-like export names including traversal attempts and long filenames
